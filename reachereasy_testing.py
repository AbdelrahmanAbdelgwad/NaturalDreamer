...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate Dreamer on Reacher-Easy"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="YAML config file used for training (e.g. reacher-easy.yml)",
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of evaluation episodes per (checkpoint, noise_sigma)",
    )
    parser.add_argument(
        "--max_episode_steps",
        type=int,
    ...
    parser.add_argument(
        "--output_csv",
        type=str,
        default="eval_results/eval_reacher.csv",
...
def set_global_seed(seed: int):
    """Use same seeding helper as training + numpy/torch."""
    seedEverything(seed)
    np.random.seed(seed)
    try:
...
def is_reacher_failure(ep_length: int, max_episode_steps: int) -> bool:
    """
    For Reacher-Easy there is no clear 'catastrophic failure' like Cartpole falling.
    For now we treat all episodes as non-failures and only use reward as a metric.
    You can change this if you define a more specific failure criterion.
    """
    return False
...
def build_env_and_dreamer(config_file: str):
    """
    Build the DMControl Reacher-Easy environment and Dreamer agent
    using the same config that was used for training.
    """
    cfg = loadConfig(config_file)

    if "CarRacing" in cfg.environmentName:
        raise ValueError(
            "This eval script is for DMControl Reacher-Easy, not CarRacing."
        )

    # DMControl env: environmentName like "reacher-easy"
...
def run_eval_reacher(
    ...
    for ep in range(num_episodes):
        ...
        failed = is_reacher_failure(stepCount, max_episode_steps)
        ...
        print(
            f"[Reacher | Ep {ep+1}/{num_episodes}] "
            f"σ={noise_sigma:.2f} | Return={currentScore:.2f}, "
            f"Len={stepCount}, Failed={failed}, "
            f"ActNormMean={action_norm_mean:.3f}"
        )
    ...
    print(f"[Eval] Saved {len(results)} Reacher episodes to {output_csv}")
