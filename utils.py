...
def findFile(filename):
    ...
    raise FileNotFoundError(
        f"File '{filename}' not found in subdirectories of {currentDir}"
    )
...
def plotMetrics(filename, title="", savePath="metricsPlot", window=10):
    ...
    fig.update_layout(
        ...
        xaxis=dict(title="Gradient Steps", showgrid=True, zeroline=False, position=0),
        yaxis_title="Value",
...
def sequentialModel1D(
    inputSize,
    hiddenSizes,
    outputSize,
    activationFunction="Tanh",
    finishWithActivation=False,
):
...
class Moments(nn.Module):
    def __init__(
        self, device, decay=0.99, min_=1, percentileLow=0.05, percentileHigh=0.95
    ):
        super().__init__()
        self._decay = decay
